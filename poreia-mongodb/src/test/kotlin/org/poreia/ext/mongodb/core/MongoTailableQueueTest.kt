...
import org.poreia.ext.mongodb.Address
import org.poreia.ext.mongodb.User
...
class MongoTailableQueueTest {
    ...
    fun setUp() {
        queue = MongoTailableCursorQueue<User>(
            mongodb.client, randomUUID().toString(), randomUUID().toString(),
    ...
    fun `it should work with value with int`() {
        ...
        val queue = MongoTailableCursorQueue<Value>(
            mongodb.client, "test", "ints", opts = QueueOpts(maxSize = 10)
        ).init()
